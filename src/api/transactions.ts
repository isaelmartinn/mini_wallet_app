...
const getRandomOutcome = (): {
  ...
  if (
    random <
    TRANSACTION_OUTCOME_PERCENTAGES.SUCCESS +
      TRANSACTION_OUTCOME_PERCENTAGES.INSUFFICIENT_FUNDS
  ) {
    return {
      success: false,
  ...
  if (
    ...
    TRANSACTION_OUTCOME_PERCENTAGES.SUCCESS +
      TRANSACTION_OUTCOME_PERCENTAGES.INSUFFICIENT_FUNDS +
      TRANSACTION_OUTCOME_PERCENTAGES.NETWORK_ERROR
  ) {
  ...
  if (
    ...
    TRANSACTION_OUTCOME_PERCENTAGES.SUCCESS +
      TRANSACTION_OUTCOME_PERCENTAGES.INSUFFICIENT_FUNDS +
      TRANSACTION_OUTCOME_PERCENTAGES.NETWORK_ERROR +
      TRANSACTION_OUTCOME_PERCENTAGES.TIMEOUT
  ) {
  ...
  sendMoney: async (
    ...
    const errorMessages: Record<TransactionErrorType, string> = {
      ...
      [TransactionErrorType.TIMEOUT]:
        'La transacción tardó demasiado. Intenta de nuevo',
